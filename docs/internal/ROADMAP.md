...
|---|-----|----------------|----------|---------------|
| G16 | No actual training run — zero reward curves, zero before/after episodes | KubeSRE: 3 training runs with reward curves + failure analysis; BioExp: trajectory datasets; EcomRLVE: trajectory generation | CRITICAL | 20% (Showing Improvement) + 30% (Storytelling) — **now addressed in Push 7 pre-training + Push 8 refinement** |
...
> **IMPORTANT:** Organisers confirmed pre-training can be completed before onsite (Apr 25). Use **Kaggle** (30 hrs/week free T4/P100) as primary training platform, **Colab free tier** as backup. HuggingFace H100 credits are provided onsite for refinement/scaling runs — NOT the first training run.
...
**Goal:** Deploy to HF Spaces, complete first GRPO training runs on free-tier GPUs, and arrive onsite with real reward curves + episode transcripts already in hand. Onsite time is then 100% refinement on H100 + deliverables.

### Free GPU Platform Comparison

| Platform | GPU | VRAM | Weekly Quota | Session Limit | Stability | Verdict |
|----------|-----|------|-------------|---------------|-----------|---------|
| **Kaggle** | T4 ×2 or P100 | 16 GB | **30 hrs/week** (sometimes higher) | 12 hrs/session | High — no random disconnects | **PRIMARY — best quota + stability** |
| **Colab Free** | T4 | 15 GB | Dynamic / unpublished | 12 hrs max | Medium — sessions can terminate randomly | **BACKUP — use if Kaggle quota exhausted** |
| **Colab Pro** ($10/mo) | L4 (24 GB) or A100 (40 GB) | 24–40 GB | Pay-as-you-go compute units | 24 hrs | High | Optional — can run 3B/7B if purchased |
| **Lightning.ai Free** | L4 | 22 GB | 4 hrs/day | N/A | Medium | Not enough quota for sustained training |

**Decision: Kaggle is the primary platform.** 30 hrs/week of T4 is more than enough for 50–100 episodes with Qwen2.5-1.5B + Unsloth 4-bit. P100 (16 GB) is also available and slightly better for training throughput. Colab free tier serves as fallback if Kaggle quota is exhausted.

**VRAM Budget on T4 (16 GB) with Unsloth 4-bit:**

| Model | Model Weights | LoRA Adapters | Optimizer States | KV Cache | Total | Fits T4? |
|-------|--------------|---------------|-----------------|----------|-------|----------|
| Qwen2.5-1.5B (4-bit) | ~1.2 GB | ~0.1 GB | ~0.4 GB | ~2 GB | **~3.7 GB** | ✅ Comfortably |
| Qwen2.5-3B (4-bit) | ~2.2 GB | ~0.2 GB | ~0.8 GB | ~3 GB | **~6.2 GB** | ✅ Yes |
| Qwen2.5-7B (4-bit) | ~4.5 GB | ~0.4 GB | ~1.5 GB | ~4 GB | **~10.4 GB** | ⚠️ Tight — may OOM with long sequences |

**Recommended: Start with 1.5B on Kaggle (guaranteed to fit), then try 3B if quota allows.**

### Phase A: Deploy + Smoke-Test (Apr 22, Day 1)
...
- Prepare Kaggle notebook (`train_kaggle.ipynb`) adapted from `train_colab.ipynb`: remove Colab-specific installs, add Kaggle dataset mounts, configure for P100/T4, keep HTTP-mode env connection to HF Space.
...
- Add `--dry-run` mode to `train.py` that runs 2 episodes with a random policy to verify the full pipeline: model load → rollout → reward computation → CSV log → plot generation.
...
- `--model-size` flag working for 1.5b / 3b / 7b presets.

### Phase B: Pre-Training on Kaggle (Apr 22–24)

> **This is the critical new addition.** Organisers confirmed pre-training can be completed before onsite. Arriving with real reward curves gives us a massive advantage.

#### Roopal: tasks (monitor + document)

- **(G20)** Test both `train_colab.ipynb` (Colab free T4) and `train_kaggle.ipynb` (Kaggle T4/P100) end-to-end with 1.5B model, 5 episodes each. Confirm both produce valid reward CSV and JSONL transcripts.
...
- After Run 1, run `python plot_rewards.py` to check signal quality. If positive slope: proceed to Run 2 (50 episodes). If flat: debug reward weights with Suyash.
...
- Update `docs/pitch_notes.md` with real reward numbers and episode excerpts (replace `[INSERT ...]` placeholders).

#### Suyash: tasks (debug + iterate)

- Support Roopal on Run 1 debugging: fix any integration bugs, reward miscalculations, or OOM errors discovered during training.
...
- Reward curve shows *some* signal (positive slope, even if noisy — this is expected for 20 episodes).
- At least 3 episode transcripts extracted for demo (failure / breakthrough / success).
- `eval_compare.py` run: base vs pre-trained comparison saved.
...
- **Session 1 (Apr 22 evening):** Dry-run + Run 1 (20 episodes, 1.5B). ~4 hrs.
- **Session 2 (Apr 23):** Run 2 (50 episodes, 1.5B) OR Run 2 (20 episodes, 3B). ~6 hrs.
...
- **Fallback:** If Kaggle has issues, switch to Colab free tier for 1.5B runs (same notebook, minor installs diff).

### PR name: `[Push 7] Roopal: HF deploy, Kaggle training, grounding, training docs` / `[Push 7] Suyash: Docker validation, dry-run mode, model-size flag, training support`

---

## Push 8 — Onsite Refinement + Deliverables (Apr 25–26, Scaler campus)

**Goal:** Scale up training on H100 (larger model, more episodes), polish all deliverables with real data, and prepare pitch. We arrive with pre-training results from Push 7 (Kaggle 1.5B runs), so onsite time is focused on scaling + storytelling — NOT debugging from scratch.

> **Compute:** HuggingFace H100 credits provided onsite (80 GB VRAM). Kaggle/Colab pre-training gives us baseline results to build on.
...
- **(G16 continued)** Launch **H100 training runs** to scale up from Kaggle pre-training:
  - **Run H1 (scale model):** `python train.py --model-size 3b --model-path Qwen/Qwen2.5-3B-Instruct --episodes 50 --seed 42` — upgrade from 1.5B to 3B leveraging H100's 80 GB VRAM. No 4-bit quantization needed — use BF16 natively.
  - **Run H2 (scale episodes, optional):** If 3B shows good signal, run 100 episodes. If 3B struggles, try `--model-size 7b` with 20 episodes — H100 can handle full Qwen2.5-7B in BF16.
  - Target: demonstrably **better** reward curves than pre-training runs (steeper slope, higher final reward, or successful on harder scenarios).
- Log every episode to CSV (`logs/rewards_h100.csv`) + JSONL (`logs/transcripts_h100.jsonl`).
- After each run, compare reward curves: Kaggle 1.5B vs H100 3B/7B. This comparison IS part of the storytelling.
- **(G21 continued)** Document any new bugs or surprises discovered during H100 training. Add to `docs/training_log.md`.

### Phase 2: Final Eval + Comparison (Day 1, Hours 4–8)

- Run `python eval_compare.py` with all checkpoints:
  - Base (random) vs Kaggle-1.5B vs H100-3B (vs H100-7B if available)
...
- Run final `eval_compare.py` focusing on curriculum progression: did the agent reach higher tiers on H100 runs?
...
- Fill `docs/pitch_notes.md` with real reward numbers and episode excerpts from H100 runs (upgrade from Push 7 Kaggle placeholders).
...
- At least 2 training runs (Kaggle pre-training + H100 refinement) with reward CSVs + combined reward curve plot.
...
| Kaggle quota runs out mid-training | Push 7 | Low | Medium | 30 hrs/week is generous. Switch to Colab free tier as backup. |
| Kaggle session disconnects during long run | Push 7 | Medium | Medium | Save checkpoints every 10 episodes. Resume from last checkpoint on reconnect. |
...
| 3B/7B model OOMs on H100 with env server | Push 8 | Low | Medium | H100 has 80 GB VRAM — even 7B BF16 uses ~14 GB. Start with 3B if cautious. |
| GRPO shows no improvement over pre-training | Push 8 | Medium | High | More episodes helps. Try different model size. Document the comparison — even negative results are a story. |
...
| Docker image fails on HF Spaces | Push 7 | Medium | High | Validated before pre-training. Have local demo backup for pitch. |
| TRL version incompatibility with vLLM | Push 7 | Medium | Medium | Pinned in pyproject.toml. Tested in `--dry-run`. |
| Colab notebook fails on T4 free tier | Push 7 | Medium | Low | Primary platform is Kaggle. Colab is backup / demo for judges. |
...
### Push 7 (Deploy + Pre-Training) — Before Apr 25
...
- [ ] `train_kaggle.ipynb` created and tested.

#### Phase B: Pre-Training
- [ ] Kaggle notebook runs on T4/P100 with 1.5B model without OOM.
- [ ] Colab notebook also tested on free tier (T4) as backup.
...
### Push 8 (Onsite Refinement) — Apr 25–26
