...
An RL agent learns to design clinical trials via GRPO, interacting with a simulator that holds hidden ground truth. All verification is objective — statistical power, FDA compliance, and trial simulation outcomes are computed mathematically.
...
- Holds `TrialLatentState` (hidden ground truth) — the true drug properties the agent must discover
- Executes agent's trial design against hidden parameters
- Returns p-values, confidence intervals, adverse event rates
- Uses real `scipy.stats` for power calculations and statistical tests — same math used in actual trial design software

### TransitionEngine (`server/simulator/transition_engine.py`)
- Separates hidden state mutation from observation generation for clean architecture
...
- Degrades data quality on soft violations (e.g., protocol deviations reduce measurement precision)
...
- Per-scenario-type mastery tracking (70% success over sliding window = graduated)
- Fast-track: 90%+ success after 3 episodes → skip min_episodes requirement
...
- All generated scenarios validated as solvable within step budget
...
- Prevents overfitting to specific parameter values — forces generalization across randomized scenarios
...
- Layer 2 (persona-scaled LLM): workflow quality assessment, justification quality scoring
  - Junior persona (difficulty < 0.4): lenient, gives hints in observation
...
- Programmatic layer is authoritative — LLM layer is informational only, never overrides
...
EpisodeTranscript (JSONL — one line per step)
├── episode_id: str
├── step: int
├── action: TrialAction
...
│   │   └── adversarial_designer.py # Expert-tier compound challenge generation
